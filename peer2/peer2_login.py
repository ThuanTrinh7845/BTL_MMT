...
import socket
import threading
from peer2 import PeerClient

class Peer2LoginApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Peer2 Login")
    ...
    def attempt_login(self):
        ...
        # Khởi tạo PeerClient cho peer2
        self.peer_client = PeerClient(self.server_ip, self.server_port, self.get_host_ip(), 33358, username=username, password=password, is_visitor=False)
    ...
    def open_main_app(self, username, is_visitor):
        from peer2_main_app import Peer2MainApp
        main_window = tk.Tk()
        app = Peer2MainApp(main_window, username, self.peer_client, is_visitor)
        main_window.mainloop()

if __name__ == "__main__":
    root = tk.Tk()
    app = Peer2LoginApp(root)
    root.mainloop()