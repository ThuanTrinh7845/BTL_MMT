...
def broadcast_message(channel_id, message, timestamp, author):
    # Gửi tin nhắn đến tracker để đồng bộ
    tracker = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tracker.connect((tracker_ip, tracker_port))
    tracker.send(f"SYNC_CHANNEL {channel_id} {timestamp} {author} {message}".encode())
    tracker.recv(1024)  # Nhận phản hồi từ tracker (giả định)
    tracker.close()
    ...
    if channel_id in channel_peers:
        for peer_ip, peer_port in channel_peers[channel_id]:
            try:
                ...
                peer_socket.send(f"NEW_MESSAGE {channel_id} {timestamp}:{author}:{message}".encode())
                peer_socket.close()
...
def peer_client(tracker_ip, tracker_port, peer_ip, peer_port, username, session_id):
    ...
    # Tạo kênh (ví dụ)
    tracker.send(f"CREATE_CHANNEL channel1 {username} {session_id}".encode())
    create_response = tracker.recv(1024).decode()
    print(create_response)

    if create_response == "CHANNEL_CREATED":
        hosted_channels["channel1"] = []
        print("channel1 is created in peer1")
    ...
    while True:
        ...
        if cmd.startswith("/join_channel"):
            ...
            # if channel_id in hosted_channels:  # Peer1 là hosting peer, thêm tin nhắn trực tiếp vào hosted_channels
            #     hosted_channels[channel_id].append((int(time.time()), username, message))
            # if channel_id in joined_channels:
            #     joined_channels[channel_id].append((int(time.time()), username, message))
            ...
            if updates.startswith("CHANNEL_UPDATES"):
                _, channel_id, messages_str = updates.split(" ")
                if messages_str:
